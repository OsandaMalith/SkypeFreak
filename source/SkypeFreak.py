...
class SkypeJunk(object):
    ...
    def _get_contacts(self):
        ...
        for count,row in enumerate(data):
            self.details[count] = {}
            self.details[count]['[+] User          '] = str(row[0])
            self.details[count]['[+] Skype Username'] = str(row[1])
            if row[2]:
                self.details[count]['[+] Country       '] = str(row[2])
            elif row[3]:
                self.details[count]['[+] City    '] = str(row[3])
            elif row[4]:
                self.details[count]['[+] About        '] = str(row[4])
            elif row[5]:
                self.details[count]['[+] Mobile No    '] = str(row[5])
            elif row[6]:
                self.details[count]['[+] Homepage'] = str(row[6])
            elif row[7]:
